...
{
    public class CommentsCell : DataGridViewTextBoxCell
    {
        public CommentsCell()
            : base()
        {
        ...
        public CommentsCell(int leftCol, int rightCol, Color background)
            : this()
        {
            this.m_nLeftColumn = leftCol;
            this.m_nRightColumn = rightCol;
        ...
        protected override void Paint(Graphics graphics, Rectangle clipBounds, Rectangle cellBounds, int rowIndex, DataGridViewElementStates cellState, object value, object formattedValue, string errorText, DataGridViewCellStyle cellStyle, DataGridViewAdvancedBorderStyle advancedBorderStyle, DataGridViewPaintParts paintParts)
        {
            try
            {
                ...
                // Pen pen = new Pen(this._background);

                // Draw the background
                graphics.FillRectangle(new SolidBrush(this._background), cellBounds);

                // Draw the separator for rows
                graphics.DrawLine(new Pen(new SolidBrush(this._background)), cellBounds.Left, cellBounds.Bottom - 1, cellBounds.Right, cellBounds.Bottom - 1);
                ...
                graphics.DrawString(strText, new Font("Arial", 10, FontStyle.Regular), new SolidBrush(SystemColors.ControlDark), rectDest, sf);
            }
            catch (Exception ex)
            {
